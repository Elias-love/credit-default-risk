...
| 冻结测试集 AUC | **0.867** |
| 冻结测试集 KS | **0.576** |
| 5 折 CV 稳定性 | 0.865 ± 0.005 |
| 概率校准 | Brier **0.138 → 0.049** |
...
4. **无泄漏实验设计**:清洗统计量仅在训练折拟合；独立验证集完成树数选择、Isotonic 校准和风险阈值冻结；测试集只用于最终一次评估
5. **可解释性落地**:SHAP 单客户归因,三个代表案例(高危/边界/白名单)展示"每个模型决策可追溯到具体特征",回应财务/审计场景对黑箱模型的质疑
6. **业务闭环**:冻结阈值后五级风险分层(E 层实际违约率 55.9% vs A 层 0.8%)→ 三项可落地财务策略
...
- 已用独立验证集完成 Isotonic 校准；若迁移到新客群/新时间窗口，仍需重新校准并监控 Brier/PSI
